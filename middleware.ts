...
export function middleware(req: NextRequest) {
  ...
  const isPrefetch = req.headers.get('purpose') === 'prefetch' || req.headers.has('next-router-prefetch');

  const isLocalhost = req.nextUrl.hostname === 'localhost' || req.nextUrl.hostname === '127.0.0.1';
  const prefix = isLocalhost ? '' : '__Host-';
  const tokenName = `${prefix}__convexAuthJWT`;

  const tokenCookie = req.cookies.get(tokenName) ?? req.cookies.get('__convexAuthJWT');
